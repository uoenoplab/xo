...
```
### Configure hostnames
Ceph may use hostnames to connect between components in different servers. Configure them in `/etc/hosts` on every hosts. For example:
```
...
```
## Configure monitor
Create a user called `ceph`. Create required folders. Do this on every hosts excep the client.
...
Create `/etc/ceph/ceph.conf` with the following. Replace the IP addresses (e.g., `192.168.11.153` to the monitor's IP, `192.168.11.0/24` to the cluster network) and hostnames (e.g., `n04` for the monitor's hostname) as appropriate. This file should be present on every hosts except the client host.
```
[global]
...
Run the following on the monitor host (`n04`). Without doing this would cause issues later on with the OSD and Rados Gateway. Replace the IP (`192.168.11.153`) and hostnames (`n04`) when running the following.
```bash
# ceph config set global osd_class_dir /usr/local/lib/rados-classes
# export PYTHONPATH=/usr/local/lib/python3.10/dist-packages:/usr/local/lib/python3/dist-packages
```
...
```
Repeat this on all the OSD hosts. In the end, `ceph -s` on either hosts should return how many OSDs are up:
```bash
...
...
```
## Setup Ceph manager
Setup the Ceph manager on the same host that runs the monitor (e.g., `n04`). Create the key for the manager, replace any IP addresses and hostnames when running the following.
```bash
mkdir -p /var/lib/ceph/mgr/ceph-n04
ceph auth get-or-create mgr.$name mon 'allow profile mgr' osd 'allow *' mds 'allow *'
```
Put the output on the screen in `/var/lib/ceph/mgr/ceph-n04/keyring`.
Start the manager on the same machine tha runs the monitor (e.g., `n04`).
```bash
...
```
## Test librados
Test if librados work on the gateway host (e.g., `n05`). With the following.
```python3
import rados
...
vim ~/.aws/credentials
```
Put the following in the file. Replace the `...` wth the `access_key` and `secret_key` you obtained from the on-screen output.
```
[default]
aws_access_key_id = ...
...
pip3 install boto3
```
Run the following in Python3 from the client host, replace the gateway hostname (e.g., `n05`) with the actual one.
```python3
import boto3

s3 = boto3.resource('s3', use_ssl=True, verify=False, endpoint_url='https://n05')
...
print(bucket.delete())
```
Running the script should return the following. All operations should be successful with a warning about self-signed certificate.
```bash
# python3 test.py 
...
```
## Reboot
The services need to be restarted if the cluster, or part of the cluster, is rebooted. Restart the services in order. First restart the monitor and manager on the monitor host.
### Monitor and Manager
```bash
...
```
### OSD
Then restart the OSDs one by one by SSH into them. First, check the OSD ID and the FSID using `ceph-volume`.
```bash
...
### Rados Gateway
Finally, restart the gateway service.
```bash
