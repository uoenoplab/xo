...
int init_forward(const char *interface_name, const char *ingress_qdisc, const char *egress_qdisc)
{
	...
	sprintf(map_path, MAP_PATH, hostname);
...
//#ifdef DEV
	fprintf(stderr, "INFO: libforward: device: %s; map: %s; ingress qdisc ID: %s; egress qdisc ID: %s; MAC: %02x:%02x:%02x:%02x:%02x:%02x; IP: %s\n",
		device_name, map_path, ingress_qdisc_parent, egress_qdisc_parent, my_mac[0], my_mac[1], my_mac[2], my_mac[3], my_mac[4], my_mac[5], inet_ntoa(my_ip.sin_addr));
//#endif
	initialized = 1;
	return 1;
}
